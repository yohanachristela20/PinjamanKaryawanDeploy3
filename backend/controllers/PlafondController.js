...
export const createPlafond = async(req, res) => {
    ...
    try {
        ...
        let plafondBaru = parseFloat(plafondTerakhir.plafond_saat_ini || 0) + jumlahPlafond;
...
export const updatePlafond = async(req, res) => {
    const transaction = await db.transaction();
    ...
    try {
        ...
        if (plafondTerakhir.plafond_saat_ini > jumlahPlafond) {
             plafondBaru = parseFloat(plafondTerakhir.plafond_saat_ini || 0) - jumlahPlafond;
        } 
        else{
            plafondBaru = parseFloat(plafondTerakhir.plafond_saat_ini || 0) + jumlahPlafond;
        ...
        await transaction.commit();
        res.status(200).json({msg: "Data Plafond berhasil diperbarui", data: {editPlafond}}); 
    } catch (error) {
        // await transaction.rollback();
        res.status(500).json({message: error.message}); 
    }
...
export const deletePlafond = async(req, res) => {
    const transaction = await db.transaction();
    try {
        const { jumlah_plafond } = req.body;
        let jumlahPlafond = parseFloat(jumlah_plafond);
        ...
        const cancelPlafond = await Plafond.destroy({
            where:{
                id_plafond: req.params.id_plafond
            }, transaction
        });
        ...
        console.log("Plafond terakhir sebelum cancel: ", plafondTerakhir.plafond_saat_ini);
        ...
        await transaction.commit();
        res.status(200).json({msg: "Data plafond berhasil dihapus", data: {cancelPlafond, plafondBaru}}); 
    } catch (error) {
        res.status(500).json({message: error.message}); 
    }
}
